...
void MainWindow::on_action_2_triggered() {
    QString xmlText = "\tРазработчик:\n\nГруппа - ИП-114\n\nСтудент: Штапов Иван Николаевич\n\n\Почта для связи: vannogun@gmail.com\n\n"
...
void MainWindow::loadImagesFromDatabase()
{
    ...
    while (query.next()) {
        ...
        QLineEdit *nameLineEdit = new QLineEdit(QFileInfo(path).fileName()); // Replace QLabel with QLineEdit

        // Connect a signal to handle the text editing
        connect(nameLineEdit, &QLineEdit::editingFinished, [=]() {
            // Save the edited text to the photoPaths list
            QString editedText = nameLineEdit->text();
            photoPaths.replace(ui->listWidget->currentRow(), editedText);
        });

        // Add the QLineEdit to the layout without setting it as read-only initially
